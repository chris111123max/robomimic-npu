...
def set_gpu_mode(mode, gpu_id=0):
    ...
    if mode and torch.cuda.is_available():
        _use_gpu = True
        _accelerator_type = "cuda"
    elif mode and _ascend_npu_available():
        _use_gpu = True
        _accelerator_type = "npu"
    else:
        _use_gpu = False
        _accelerator_type = None
    device = torch.device(
        _accelerator_type + ":" + str(gpu_id) if _use_gpu else "cpu"
    )
...
def set_device(gpu_id):
    if _accelerator_type == "npu":
        torch.npu.set_device(gpu_id)
    elif _accelerator_type == "cuda":
