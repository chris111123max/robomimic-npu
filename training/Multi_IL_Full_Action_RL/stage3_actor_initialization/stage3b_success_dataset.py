"""Stage 3B read-only successful BC-RNN transition dataset."""
...
class Stage3BSuccessTransitionDataset:
    """Load successful episodes for one explicitly supplied seed split."""
        ...
        with h5py.File(self.metadata["path"], "r") as handle:
            ...
            for seed in self.requested_seeds:
                ...
                if state.shape != (action.shape[0], 59) or action.shape[1:] != (14,):
                    raise RuntimeError(
                        f"Unexpected successful transition shapes seed={seed}: "
