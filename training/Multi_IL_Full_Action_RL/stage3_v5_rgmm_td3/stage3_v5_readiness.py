...
import numpy as np
from stage3_v5_diagnostics import frozen_set, isolated_training_rng
...
def stability(history, rules):
    history = [row for row in history if row.get("diagnostic_available", True)]
    if len(history) < 3: return False, False, False
    recent = history[-3:]
    td = [row["td_mae"] for row in recent]
    relative_change = abs(td[-1] - td[0]) / max(abs(td[0]), 1e-12)
    worsening = (td[-1] > td[0] * (1 + rules["td_worsening_limit"]) or
                 (td[0] < td[1] < td[2] and relative_change > rules["td_plateau_relative_improvement"]))
    td_plateau = (relative_change <= rules["td_plateau_relative_improvement"]
                  and td[-1] <= td[0] * (1 + rules["td_plateau_relative_improvement"]) and not worsening)
...
def replay_metrics(agent, online, episodes, successes, config, history, sample_count=256, env_steps=None, offline=None):
    with isolated_training_rng(online, offline):
        return _replay_metrics(agent, online, episodes, successes, config, history,
                               sample_count, env_steps)


def _replay_metrics(agent, online, episodes, successes, config, history, sample_count, env_steps):
    ...
    """
    fixed = frozen_set(online, config, sample_count)
    if fixed is None:
        return {"env_steps": int(env_steps), "completed_episodes": int(episodes),
                "success_episodes": int(successes), "failure_episodes": int(episodes-successes),
                "spearman_q_return": 0.0, "success_failure_auc": 0.0, "delta_q": 0.0,
                "twin_q_disagreement_median": 0.0, "twin_q_disagreement_p95": 0.0,
                "td_plateau": False, "td_error_worsening": False, "q_scale_stable": False,
                "ood_stress_pass": False, "finite": True, "diagnostic_available": False}
    completed = fixed["episodes"]
    outputs = agent.q_values_for_episodes(completed)
    values, returns, labels, episode_q = [], [], [], []
    for episode, (q1, q2) in zip(completed, outputs):
        qmin_episode = np.minimum(q1, q2)
        values.extend(qmin_episode); returns.extend(discounted_returns(episode, config["gamma"]))
        episode_q.append(float(np.mean(qmin_episode)))
