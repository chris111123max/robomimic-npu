...
class StaggeredVectorEnv:
    ...
    def __init__(
        ...
        self.startup_parallelism = int(startup_parallelism)

        if self.num_envs <= 0:
            raise ValueError("num_envs must be positive")
        if self.startup_parallelism <= 0:
        ...
        try:
            for first in range(0, self.num_envs, self.startup_parallelism):
                batch = range(first, min(first + self.startup_parallelism, self.num_envs))
                for env_id in batch:
                    self._start_one(env_id, wait=False)
                for env_id in batch:
                    self._await_ready(env_id)
                if first + self.startup_parallelism < self.num_envs:
    ...
    def _start_one(self, env_id: int, wait: bool = True) -> None:
    ...
    def _await_ready(self, env_id: int) -> None:
        seed = self.seed_base + int(env_id)
        message = self._recv(
            env_id,
