...
allowed={'stage','output_root','diagnostic_sequences','total_env_steps','actor_freeze_steps','actor_warmup_end','automatic_entropy_tuning','initial_entropy_alpha','target_entropy_active','evaluation_interval','checkpoint_steps'}
...
declare -A PIDS
for G in rnn_only_critic multi_il_critic;do mkdir -p "$RUN/$G";nohup env PYTORCH_NPU_ALLOC_CONF=expandable_segments:True python -u "$V1/train_stage4_group.py" --group "$G" --device "${DEV[$G]}" --config "$CONFIG" --run-dir "$RUN" >"$RUN/$G/train.log" 2>&1 </dev/null &PIDS[$G]=$!;echo "${PIDS[$G]}" >"$RUN/$G/pid";done
...
echo "Stage4-v2 run directory: $RUN";echo "RNN PID=${PIDS[rnn_only_critic]} device=$RNN_DEVICE log=$RUN/rnn_only_critic/train.log";echo "Multi PID=${PIDS[multi_il_critic]} device=$MULTI_DEVICE log=$RUN/multi_il_critic/train.log";echo "NO RANDOM GROUP RUN; budget=100000/group; parallel_envs=8/group"
