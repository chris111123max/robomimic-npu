...
established={'stage','output_root','diagnostic_sequences','total_env_steps','actor_freeze_steps','actor_warmup_end','automatic_entropy_tuning','initial_entropy_alpha','target_entropy_active','evaluation_interval','evaluation_steps','checkpoint_steps','parallel_envs','evaluation_parallel_envs','performance_log_interval','resource_log_interval'}
for key in established:a[key]=b.get(key)
allowed={'collector_mode','schedule_clock'}
...
json.dump({'baseline':'previous Stage4-v2 with 16 env/group','established_unchanged_keys':sorted(established),'allowed_difference_keys':sorted(allowed),'differences':diff},open(out,'w'),indent=2)
...
if [[ "$MODE" == smoke ]];then
  ...
  for G in rnn_only_critic multi_il_critic;do
    [[ "$G" == rnn_only_critic ]]&&CPU_KEY=rnn_trainer_cpu_id||CPU_KEY=multi_trainer_cpu_id;TRAINER_CPU="$(python -c "import json;print(json.load(open('$RUN/cpu_affinity.json'))['$CPU_KEY'])")";mkdir -p "$RUN/$G"
    env OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1 VECLIB_MAXIMUM_THREADS=1 PYTORCH_NPU_ALLOC_CONF=expandable_segments:True taskset -c "$TRAINER_CPU" python -u "$HERE/train_stage4_v2_sync_group.py" --group "$G" --device "${DEV[$G]}" --config "$CONFIG" --run-dir "$RUN" --cpu-affinity-file "$RUN/cpu_affinity.json" --smoke-test >"$RUN/$G/smoke.log" 2>&1 &SMOKE_PIDS[$G]=$!
  done
  wait "${SMOKE_PIDS[rnn_only_critic]}";wait "${SMOKE_PIDS[multi_il_critic]}";cat "$RUN/rnn_only_critic/sync_collector_smoke.json";cat "$RUN/multi_il_critic/sync_collector_smoke.json"
  echo "STAGE4-V2 SYNC LEARNING-START SMOKE COMPLETE; NO FORMAL RUN STARTED";echo "Run directory: $RUN";exit 0
fi
declare -A PIDS
for G in rnn_only_critic multi_il_critic;do
