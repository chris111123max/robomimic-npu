...
def checkpoint_payload(agent, config, group, env_steps, generations,
    ...
    return {
        "stage": "stage3-v4", "group": group, "env_steps": int(env_steps),
...
def restore_checkpoint(path, agent, config, torch, OnlineSequenceReplay):
    ...
    if payload.get("stage") != "stage3-v4":
...
def main():
    ...
    if args.smoke:
        ...
        raise RuntimeError("Formal Stage3-v4 requires exactly 16 environments")
    ...
    try:
        ...
        from stage3_v4_agent import RecurrentGMMTD3, strict_stage2_load
        from stage3_v3_evaluation import build_env, close_env, evaluate_actor
        from stage3_v4_replay import (OfflineDemonstrations, OnlineSequenceReplay,
        ...
        def run_evaluation(step):
            ...
            report.update({"stage": "stage3-v4", "group": args.group,
            ...
            diagnostics.update({"stage": "stage3-v4", "group": args.group,
                                "env_steps": int(step)})
        ...
        if args.smoke:
            ...
            smoke_checks = {
                "checkpoint_load": roundtrip.get("stage") == "stage3-v4",
        ...
        write_json(group_dir / "summary.json", {
            "stage": "stage3-v4", "status": status, "run_type": config["run_type"],
