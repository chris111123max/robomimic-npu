...
from stage3_v4_agent import strict_stage2_load
...
def arguments():
    ...
    parser.add_argument("--config", default=str(HERE / "stage3_v4_config.json"))
...
def main():
    ...
    run_id = args.run_id or datetime.now().strftime("stage3v4_%Y%m%d_%H%M%S")
    ...
    actor_payload = {
        "stage": "stage3-v4", "source_checkpoint": str(bc_checkpoint),
    ...
    print(json.dumps({"status": "PREPARED", "pair_run_dir": str(run.resolve()),
                      ...
                      "next": ("train_stage3_v4_vector.py" if phase0_reuse
                               else "run_phase0_stage3_v4.py")}, indent=2))
