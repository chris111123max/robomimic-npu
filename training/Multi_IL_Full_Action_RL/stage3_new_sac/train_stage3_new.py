...
from stage3_new_handoff import FrozenRNNProposer,evaluate_handoff,select_actions
...
def aggregate_metrics(rows):
    last=rows[-1];result={"env_steps":last["env_steps"],"gradient_updates":last["gradient_updates"],"aggregation_updates":len(rows),"phase":last.get("phase")}
    keys=set().union(*(row.keys() for row in rows))
    for key in keys:
        if key in result or key in ("action_mean","action_std","action_saturation_rate"):continue
        values=[row[key] for row in rows if isinstance(row.get(key),(int,float)) and not isinstance(row.get(key),bool) and math.isfinite(float(row[key]))]
        if values:result[key]=float(np.mean(values))
        elif key in last and last[key] is None:result[key]=None
    return result
...
def save(path,agent,replay,sampler,offline,config,group,stage2,env_steps,episode_index,context,env,anchor_data=None,sim_fatal_error_count=0,proposer=None,selector_counts=None):
...
def load_resume(path,agent,offline,config,env,anchor_data=None,proposer=None):
    ...
    if proposer is not None and context is not None:
        if p.get("rnn_proposer_state") is None:raise RuntimeError("Cannot resume handoff mid-episode without BC-RNN hidden state")
        proposer.load_state_dict(p["rnn_proposer_state"])
...
def preserved_handoff_evaluation(actor,target,proposer,env,seeds,horizon,device_,retries=0):
    state=rng_state()
    try:return evaluate_handoff(actor,target,proposer,env,seeds,horizon,device_,retries)
    finally:restore_rng(state)
def main():
    ...
    handoff_cfg=config.get("handoff",{});handoff_enabled=bool(handoff_cfg.get("enabled",False));proposal_cache=config.get("expert_rnn_proposal_cache") if handoff_enabled else None
    actor_init_path=pair/"shared"/"actor_init.pth";seed_manifest_path=pair/"shared"/"seed_manifest.json";expert=ExpertDataset(config["expert_dataset"],config["training_seed"],proposal_cache);actor=build_actor(config,d);actor_payload=torch.load(actor_init_path,map_location=d);actor.load_state_dict(actor_payload["actor_state_dict"],strict=True)
    ...
    proposer=FrozenRNNProposer(config["bc_rnn_checkpoint"],d) if handoff_enabled else None;eval_proposer=FrozenRNNProposer(config["bc_rnn_checkpoint"],d) if handoff_enabled else None;env=build_env(config["expert_dataset"]);eval_env=build_env(config["expert_dataset"]);action_low,action_high=environment_action_bounds(env);agent=Stage3SAC(actor,critic,config,d,action_low,action_high,teacher);online=TransitionBuffer(config["online_replay_capacity"],59,14,config["training_seed"]);sampler=SymmetricSampler(expert,online,config["training_seed"])
    ...
    if handoff_enabled:print(f"[STAGE3-HANDOFF] group={a.group} proposer=bc_rnn selector=target_twin_q_min tie=rnn lambda_handoff={handoff_cfg['lambda_handoff']} bootstrap_proposal=True",flush=True)
    if progressive_enabled:print(f"[STAGE3-PROGRESSIVE] protected_until={progressive_cfg['protected_until_env_steps']} unfreeze_end={progressive_cfg['unfreeze_end_env_steps']} critic_lr_schedule=linear target_tau_schedule=linear phaseA_actor=handoff_only phaseA_alpha_fixed={progressive_cfg['phase_a_alpha_fixed']}",flush=True)
    total=int(config["total_env_steps"]);eval_steps=set(map(int,config["evaluation_env_steps"]))|{total};probe_steps=set(map(int,config["probe_env_steps"]))|{total};checkpoint_steps=set(map(int,config["checkpoint_env_steps"]))|{total};selector_steps=set(map(int,config.get("selector_diagnostic_env_steps",[])))|({total} if handoff_enabled else set());diag_interval=int(cql_cfg.get("source_diagnostic_interval_env_steps",5000));diagnostic_steps=set(range(diag_interval,total+1,diag_interval))|({1000,total} if cql_cfg.get("enabled",False) else set());anchor_diagnostic_steps=set(map(int,anchor_cfg.get("diagnostic_env_steps",[])))|({total} if anchor_enabled else set());eval_retries=int(config.get("sim_error_handling",{}).get("evaluation_retry_count",0))
    if env_steps==0:
        before=online.size;report=preserved_handoff_evaluation(actor,agent.target,eval_proposer,eval_env,seeds["evaluation_seeds"],config["horizon"],d,eval_retries) if handoff_enabled else preserved_evaluation(actor,eval_env,seeds["evaluation_seeds"],config["horizon"],d,eval_retries);assert online.size==before;write(group_dir/"evaluations"/"step_000000.json",report)
        ...
        save(group_dir/"checkpoints"/"step_000000.pth",agent,online,sampler,expert,config,a.group,actual,0,0,None,env,anchor_data,sim_fatal_error_count,proposer,selector_counts)
    try:
        while env_steps<int(config["total_env_steps"]):
            if context is None:
                train_seed=int(config["train_seed_base"])+episode_index;observation=reset_seed(env,train_seed)
                if handoff_enabled:proposer.start_episode();rnn_action=proposer.action(observation)
                context={"seed":train_seed,"observation":observation,"return":0.0,"length":0,"success":False,"rnn_action":rnn_action if handoff_enabled else None,"rnn_selected":0,"rl_selected":0};episode_index+=1
            state=flatten(context["observation"]);rl_action=agent.action(state,False)[0]
            if handoff_enabled:
                state_t=torch.as_tensor(state[None],dtype=torch.float32,device=d);rl_t=torch.as_tensor(rl_action[None],dtype=torch.float32,device=d);rnn_t=torch.as_tensor(context["rnn_action"][None],dtype=torch.float32,device=d);chosen,qrl,qrnn,rl_wins=select_actions(agent.target,state_t,rl_t,rnn_t);action=chosen[0].cpu().numpy();selected=int(rl_wins.item());qrl_value=float(qrl.item());qrnn_value=float(qrnn.item());margin=qrl_value-qrnn_value;source="rl" if selected else "rnn"
            else:action=rl_action
            ...
            if handoff_enabled:selector_counts[source]+=1;selector_window[source]+=1;selector_window["q_rl"].append(qrl_value);selector_window["q_rnn"].append(qrnn_value);selector_window["margin"].append(margin);context[f"{source}_selected"]+=1
            next_rnn_action=proposer.action(next_obs) if handoff_enabled else None
            truncated=bool(context["length"]>=int(config["horizon"]) and not won);terminal=bool((config["terminate_on_success"] and won) or (raw_done and not truncated));metadata={"action_exec":action,"action_rl":rl_action,"action_rnn":context["rnn_action"],"rnn_next_actions":next_rnn_action,"selected_source":[selected],"q_select_rl":[qrl_value],"q_select_rnn":[qrnn_value],"q_select_margin":[margin],"is_online":[1]} if handoff_enabled else None;online.add(state,action,reward,flatten(next_obs),terminal,metadata);env_steps+=1
            if progressive_enabled and not protected_asserted and env_steps==int(progressive_cfg["protected_until_env_steps"]):
                if state_hash(agent.critic)!=initial_critic_hash or state_hash(agent.target)!=initial_target_hash:raise RuntimeError("Phase A changed protected online or target Critic")
                if not math.isclose(float(agent.alpha.item()),float(progressive_cfg["phase_a_alpha_fixed"]),rel_tol=1e-6,abs_tol=1e-8):raise RuntimeError("Phase A changed fixed alpha")
                if state_hash(agent.actor)==initial_actor_hash:raise RuntimeError("Phase A Actor did not change under handoff imitation")
                protected_asserted=True;print(f"[STAGE3-PROGRESSIVE] transition protected_handoff -> progressive_unfreeze at env_steps={env_steps}",flush=True)
            if progressive_enabled and env_steps==int(progressive_cfg["unfreeze_end_env_steps"]):print(f"[STAGE3-PROGRESSIVE] transition progressive_unfreeze -> full_sac at env_steps={env_steps}",flush=True)
            metrics=None
            if online.size>=int(config["min_online_replay_size"]):
                try:metrics=agent.update(sampler.sample(config["batch_size"]),anchor_data.sample(anchor_cfg["batch_size"]) if anchor_enabled else None,env_steps)
                except FloatingPointError as error:
                    write(group_dir/"numerical_failure.json",{"env_steps":env_steps,"gradient_updates":agent.updates,"error":str(error)});save(group_dir/"checkpoints"/"numerical_failure.pth",agent,online,sampler,expert,config,a.group,actual,env_steps,episode_index,context,env,anchor_data,sim_fatal_error_count,proposer,selector_counts);raise
                ...
                latest_metrics=metrics;metric_window.append(metrics)
            if terminal or truncated:
                terminated_count+=int(terminal);truncated_count+=int(truncated);success_count+=int(won);episode_row={"episode":episode_index-1,"episode_seed":context["seed"],"episode_return":context["return"],"episode_length":context["length"],"success":bool(won),"terminated":terminal,"truncated":truncated,"rnn_selected_fraction":context["rnn_selected"]/context["length"] if handoff_enabled else None,"rl_selected_fraction":context["rl_selected"]/context["length"] if handoff_enabled else None};context=None
            ...
            else:context["observation"]=next_obs;context["rnn_action"]=next_rnn_action
            if metrics is not None:
                if not progressive_enabled:log(group_dir/"train_metrics.jsonl",metrics)
                elif len(metric_window)>=metric_interval:
                    log(group_dir/"train_metrics.jsonl",aggregate_metrics(metric_window));metric_window=[]
            ...
            if progressive_enabled and (terminal or truncated):log(group_dir/"episode_metrics.jsonl",episode_row)
            if env_steps in eval_steps:
                before=online.size;report=preserved_handoff_evaluation(actor,agent.target,eval_proposer,eval_env,seeds["evaluation_seeds"],config["horizon"],d,eval_retries) if handoff_enabled else preserved_evaluation(actor,eval_env,seeds["evaluation_seeds"],config["horizon"],d,eval_retries)
            ...
            if handoff_enabled and env_steps in selector_steps:
                margins=np.asarray(selector_window["margin"]);window_total=selector_window["rnn"]+selector_window["rl"];schedule=progressive_schedule(config,env_steps);row={"env_steps":env_steps,"phase":schedule["phase"],"critic_lr_effective":schedule["critic_lr_effective"],"target_tau_effective":schedule["target_tau_effective"],"rnn_selected_fraction_window":selector_window["rnn"]/window_total if window_total else None,"rl_selected_fraction_window":selector_window["rl"]/window_total if window_total else None,"rnn_selected_total":selector_counts["rnn"],"rl_selected_total":selector_counts["rl"],"q_select_rnn_mean":float(np.mean(selector_window["q_rnn"])) if margins.size else None,"q_select_rl_mean":float(np.mean(selector_window["q_rl"])) if margins.size else None,"q_select_margin_mean":float(margins.mean()) if margins.size else None,"q_select_margin_p10":float(np.percentile(margins,10)) if margins.size else None,"q_select_margin_p50":float(np.percentile(margins,50)) if margins.size else None,"q_select_margin_p90":float(np.percentile(margins,90)) if margins.size else None};log(group_dir/"selector_diagnostics.jsonl",row);log(group_dir/"milestone_diagnostics.jsonl",{**row,"training":latest_metrics});selector_window={"rnn":0,"rl":0,"q_rnn":[],"q_rl":[],"margin":[]}
            if env_steps in checkpoint_steps:save(group_dir/"checkpoints"/f"step_{env_steps:06d}.pth",agent,online,sampler,expert,config,a.group,actual,env_steps,episode_index,context,env,anchor_data,sim_fatal_error_count,proposer,selector_counts)
        if metric_window:log(group_dir/"train_metrics.jsonl",aggregate_metrics(metric_window))
        save(group_dir/"checkpoints"/"last.pth",agent,online,sampler,expert,config,a.group,actual,env_steps,episode_index,context,env,anchor_data,sim_fatal_error_count,proposer,selector_counts);write(group_dir/"summary.json",{"status":"COMPLETE","group":a.group,"env_steps":env_steps,"gradient_updates":agent.updates,"episodes":episode_index,"terminated_count":terminated_count,"truncated_count":truncated_count,"success_count":success_count,"sim_fatal_error_count":sim_fatal_error_count,"replay_size":online.size,"sampling_fractions":sampler.fractions(),"selector_counts":selector_counts,"anchor_sampling_proportions":anchor_data.proportions() if anchor_enabled else None,"actor_hash_final":state_hash(actor),"critic_hash_final":state_hash(critic)})
