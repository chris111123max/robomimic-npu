...
def main():
    ...
    if env_steps==0:
        ...
        summary=record_probe(actor,critic,probes,d,group_dir/"probes"/"step_000000.npz");write(group_dir/"probes"/"step_000000.json",summary);save(group_dir/"checkpoints"/"step_000000.pth",agent,online,sampler,expert,config,a.group,actual,0,0,None,env)
    try:
        while env_steps<int(config["total_env_steps"]):
            ...
            if online.size>=int(config["min_online_replay_size"]):
                metrics=agent.update(sampler.sample(config["batch_size"]));metrics.update({"env_steps":env_steps,"gradient_updates":agent.updates,"offline_batch_fraction":sampler.fractions()["offline"],"online_batch_fraction":sampler.fractions()["online"],"online_replay_size":online.size,"actual_utd_after_replay_ready":1.0})
            ...
            if env_steps in probe_steps:
                summary=record_probe(actor,critic,probes,d,group_dir/"probes"/f"step_{env_steps:06d}.npz");write(group_dir/"probes"/f"step_{env_steps:06d}.json",summary)
