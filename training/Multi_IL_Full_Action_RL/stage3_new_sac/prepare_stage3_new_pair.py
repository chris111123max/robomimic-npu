...
def main():
    a=args();c=read(a.config);c["alpha_init"]=float(a.alpha_init);cql=c.get("cql",{"enabled":False});anchor=c.get("anchor",{"enabled":False});handoff=c.get("handoff",{"enabled":False});c["experiment_tag"]="rnn_handoff_cql_lamH1p0" if handoff.get("enabled",False) else ("cqllite_anchor_zscore_lam0p1" if anchor.get("enabled",False) else ("cqllite_lam0p1" if cql.get("enabled",False) else ("alpha_init_0p01" if c["alpha_init"]==0.01 else f"alpha_init_{c['alpha_init']:g}")));c["expert_dataset"],c["expert_dataset_provenance"]=resolve_expert(a);c["stage2_run_dir"]=str(Path(a.stage2_run_dir).resolve())
