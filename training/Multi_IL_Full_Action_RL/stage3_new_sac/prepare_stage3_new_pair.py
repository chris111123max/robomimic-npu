...
def main():
    ...
    if progressive.get("enabled",False):
        ...
        parallel=c.get("parallel_env",{})
        if int(parallel.get("num_envs",-1))!=16 or not parallel.get("env_startup_stagger",False) or float(parallel.get("env_startup_delay_sec",-1))<0 or float(parallel.get("env_startup_timeout_sec",0))<=0 or parallel.get("multiprocessing_start_method")!="spawn":
            raise RuntimeError("Fixed progressive 16-env contract changed")
