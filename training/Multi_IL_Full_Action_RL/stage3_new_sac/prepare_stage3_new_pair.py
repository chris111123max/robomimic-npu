...
def main():
    ...
    write(shared/"config_resolved.json",c);write(shared/"reference_pair_manifest.json",reference);write(shared/"stage2_source_manifest.json",sources);write(shared/"expert_dataset_audit.json",expert.audit());seeds=read(shared/"seed_manifest.json");manifest={"experiment_name":c["stage"],"experiment_tag":c["experiment_tag"],"timestamp":datetime.now().isoformat(),"actor_init_path":str((shared/"actor_init.pth").resolve()),"actor_init_hash":torch.load(shared/"actor_init.pth",map_location="cpu")["actor_hash"],"rnn_q_checkpoint":sources["rnn_q"]["checkpoint"],"multi_q_checkpoint":sources["multi_q"]["checkpoint"],"cql":cql,"cql_random_seed_protocol":"shared training seed and identical vectorized draw schedule per gradient update","batch_size":c["batch_size"],"expert_fraction":c["offline_fraction"],"online_fraction":c["online_fraction"],"gamma":c["gamma"],"tau":c["tau"],"actor_lr":c["actor_lr"],"critic_lr":c["critic_lr"],"alpha_lr":c["alpha_lr"],"alpha_init":c["alpha_init"],"automatic_entropy_tuning":c["automatic_entropy_tuning"],"critic_layernorm":c["critic_layer_norm"],"critic_weight_decay":c["critic_weight_decay"],"seed":seeds["training_seed"],"eval_seeds":seeds["evaluation_seeds"]};write(shared/"run_manifest.json",manifest)
