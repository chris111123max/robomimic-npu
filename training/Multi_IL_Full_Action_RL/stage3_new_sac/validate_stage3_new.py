...
def config():
    return {"hidden_dims":[256,256],"actor_lr":3e-4,"critic_lr":3e-4,"alpha_lr":3e-4,"critic_weight_decay":1e-4,"gamma":.99,"tau":.005,"target_entropy":-14.0,"initial_alpha":1.0,"training_seed":7}
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="stage3_new_smoke_") as tmp:
        ...
        agent=Stage3SAC(actor1,critic,c,"cpu");assert all(torch.equal(agent.target.state_dict()[k],target_copy[k]) for k in target_copy);names=[type(x).__name__ for x in agent.critic.q1.network];assert names==["Linear","LayerNorm","ReLU","Linear","LayerNorm","ReLU","Linear"] and agent.critic_optimizer.param_groups[0]["weight_decay"]==1e-4
        ...
        print(json.dumps({"status":"PASS","offline_online_fractions":fractions,"checks":26}))
